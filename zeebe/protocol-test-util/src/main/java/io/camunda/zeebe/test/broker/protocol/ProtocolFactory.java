...
/**
 * A {@link Record} factory which produces randomized records deterministically. A seed can be given on construction to reproduce the same records. On failure, the seed can be fetched via
 * {@link #getSeed()}. By default, calling {@link ProtocolFactory#ProtocolFactory()} will always return a factory which produces the same records. This is useful for reproducible tests, such that
 * running the same test twice (regardless of the environment, e.g. CI or locally) will produce the same results.
 *
 * <p>Every property is fully randomized and cannot be relied upon semantically, except the value
 * type, the value, and the intent. These will always all match the implicit assumptions of the protocol. For example, if the {@link ValueType} is {@link ValueType#JOB}, then the record's value is
 * going to be an instance of {@link io.camunda.zeebe.protocol.record.value.JobRecordValue}, and the intent is going to be an instance of {@link io.camunda.zeebe.protocol.record.intent.JobIntent}.
 * This is done to enable using this factory with tests that expect to serialize and deserialize the records.
 */
...
public final class ProtocolFactory {
  ...
  /**
   * Every call to this constructor will always return a factory which produces the exact same record. This is useful for reproducible tests. If you want a different behavior, then you can use
   * {@link ProtocolFactory#ProtocolFactory(long)} and pass something like {@link ThreadLocalRandom#nextLong()}.
   */
  ...
  /**
   * Returns a factory with the given seed for the initial randomization. This enables you to reproduce past results if you know the seed. Note that you will need to also effectively reproduce the
   * same steps as in the past; the seed by itself is not enough. So if you have a seed, and want to reproduce the 10th record generated, you will need to generate 10 records again.
   *
   ...
   *
   * @param modifier applied to the builder after all the properties have been filled as the last step; cannot be null
   * @return a stream of random records
  ...
  /**
   * Generates a stream of records, one record for each known {@link ValueType} (except the values created by SBE, i.e. NULL_VAL and SBE_UNKNOWN).
   *
  ...
  /**
   * Generates a record with the given {@code valueType}. The value and intent properties will be picked from the appropriate types based on the given {@code valueType}. This means, if you pass
   * {@link ValueType#ERROR}, the value will be of type {@link io.camunda.zeebe.protocol.record.value.ErrorRecordValue}, and the intent of type
   * {@link io.camunda.zeebe.protocol.record.intent.ErrorIntent}. Each of these will, of course, be randomly generated as well.
   *
   * @param valueType the expected value type of the record
   * @return a randomized record with the given value type, with the value and intent being of the expected types
  ...
  /**
   * Generates a record with the given {@code valueType} and {@code intent}. The value will be picked from the appropriate types based on the given {@code valueType}. This means, if you pass
   * {@link ValueType#ERROR}, the value will be of type {@link io.camunda.zeebe.protocol.record.value.ErrorRecordValue}, and the intent of type
   * {@link io.camunda.zeebe.protocol.record.intent.ErrorIntent}. Each of these will, of course, be randomly generated as well.
   *
   * @param valueType the expected value type of the record
   * @param intent the expected intent of the record (if it is null, a random intent will be chosen)
   * @return a randomized record with the given value type, with the value being of the expected types
   * @throws NullPointerException if {@code valueType} is null
   */
  public <T extends RecordValue> Record<T> generateRecordWithIntent(final ValueType valueType, final Intent intent) {
  ...
  /**
   * Generates a record with the given {@code valueType}. The value property will be picked from the appropriate types based on the given {@code valueType}. This means, if you pass
   * {@link ValueType#ERROR}, the value will be of type {@link io.camunda.zeebe.protocol.record.value.ErrorRecordValue}, and the intent of type
   * {@link io.camunda.zeebe.protocol.record.intent.ErrorIntent}. Each of these will, of course, be randomly generated as well.
   *
   * <p>The given modifier is applied to the final builder as the last step of the generation.
  ...
  /**
   * Generates a random object. Can be used, for example, to generate a random {@link io.camunda.zeebe.protocol.record.value.ErrorRecordValue}.
   *
  ...
  /**
   * A few of the protocol types refer to {@code Map<String, Object>}. easy-random doesn't really know how to randomize the object type, and instead just creates raw {@link Object} instances. This not
   * only doesn't really simulate the expected data set, but it's not serializable, which would prevent this class from being used in any kind of serialization context (e.g. exporters)
   *
   ...
   * and instead will always deserialize them as {@link java.math.BigDecimal}. This is a regression, so we could think about adding them back once that's fixed, but it didn't seem super important at
   * the moment.
   */
